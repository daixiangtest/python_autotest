...
from hamster_test.comms.constants import DATA_APTOS
...
@allure.feature("Aptos项目合约")
@allure.parent_suite('Aptos项目合约')
class TestAptos:
    cases = get_yaml_data(DATA_APTOS)  # 读取yaml文件中的测试数据
    ids = ['测试{}'.format(case['case_title'][0]) for case in cases]

    @allure.suite('APTOS生态创建合约')
    @allure.description("APTOS生态创建合约")
    ...
    # @pytest.mark.skip(reason="测试通过跳过")
    @allure.suite('aptos生态检查合约')
    @allure.description("aptos生态检查合约")
    ...
    # @pytest.mark.skip()
    @allure.suite('Aptos生态构建合约')
    @allure.description("Aptos生态构建合约")
    ...
    @allure.suite('Aptos生态删除合约')
    @allure.description("Aptos生态删除合约")
    ...
    def test_delete(self, case, connect_db):
        ...
        connect_db.refresh()
        # 判断被删除的元素在该项目中是否还存在
        ast1 = cp.iselement(5, By.CSS_SELECTOR,
                            'a[href="https://github.com/{}.git"]'.format('{}/{}'.format(sth, case['case_data'])))
        if lens - case["case_id"] + 1 > 10 and ast1 is False:
            cp.find_elements(5, *em.project_switch_pages)[1].click()
            ast1 = cp.iselement(5, By.CSS_SELECTOR,
                                'a[href="https://github.com/{}.git"]'.format('{}/{}'.format(sth, case['case_data'])))
        try:
            assert ast is False
