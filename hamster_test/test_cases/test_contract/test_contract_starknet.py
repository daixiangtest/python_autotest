...
from hamster_test.comms.constants import DATA_STARKNET
...
@allure.feature("starknet项目合约")
@allure.parent_suite('starknet项目合约')
class TestStarknet:
    cases = get_yaml_data(DATA_STARKNET)  # 读取yaml文件中的测试数据
    ...
    @allure.suite('starknet生态检查合约')
    @allure.description("starknet生态检查合约")
    @pytest.mark.parametrize('case', cases, ids=ids)
    def test_check(self, case, connect_db):
        ...
        lens = len(get_yaml_data(DATA_STARKNET))
        # 登录项目
        cp.longin(get_ini_data('github', 'user'), get_ini_data('github', 'passwd'))
        # 点击项目详情按钮
    ...
    #
    # @allure.suite('starknet生态构建合约')
    # @allure.description("starknet生态构建合约")
    ...
    #
    # @allure.suite('starknet生态部署合约')
    # @allure.description("starknet生态部署合约")
    ...
    @allure.suite('starknet生态删除合约')
    @allure.description("starknet生态删除合约")
