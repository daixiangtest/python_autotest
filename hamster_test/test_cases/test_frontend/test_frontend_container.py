...
class TestFrontEnd:
    cases = get_yaml_data(DATA_CONTAINER)  # 读取yaml文件中的测试数据
    ...
    #     # 打开ipfs合约的展示页面
    #     cp.open_frontend_container()
    #     # 点击对应的合约
    #     cp.click(10, *em.frontend_container[case['case_id'] - 1])
    ...
    #         get_picture(self.driver, case['case_title'][0])  # 失败截图
    #         logger.error(
    ...
    #     except AssertionError as e:
    #         get_picture(self.driver, case['case_title'][1])  # 失败截图
    #         logger.error(
    ...
    #     cp.frontend_projects(case['case_data'], lens)
    #     # 点击项目build按钮
    #     cp.clicks(10, *em.projects_build)
    #     # 判断代码是否build成功
    ...
    @allure.suite('Container生态删除合约')
    @allure.description("Container生态删除合约")
