...
from hamster_test.comms.constants import DATA_ICP
...
@allure.feature("ICP项目合约")
@allure.parent_suite('ICP项目合约')
class TestFrontEnd:
    cases = get_yaml_data(DATA_ICP)  # 读取yaml文件中的测试数据
    ids = ['测试{}'.format(case['case_title'][0]) for case in cases]

    @allure.suite('ICP生态创建合约')
    @allure.description("ICP生态创建合约")
    ...
    # ids1 = ['测试{}'.format(case['case_title'][0]) for case in cases]
    #
    # @allure.suite('ICP生态检查合约')
    # @allure.description("ICP生态检查合约")
    ...
    #     lens = len(get_yaml_data(DATA_ICP))
    #     # 登录项目
    #     cp.longin(get_ini_data('github', 'user'), get_ini_data('github', 'passwd'))
    ...
    @allure.suite('ICP生态构建合约')
    @allure.description("ICP生态构建合约")
    ...
    @allure.suite('ICP生态部署合约')
    @allure.description("ICP生态部署合约")
    ...
    def test_deploy(self, case, connect_db):
        ...
        # 判断是否需要配置文件
        if cp.iselement(10, *em.icp_configue_done):
            cp.click(10, *em.icp_configue_done)
        # 等待deploy成功
        cp.wait_recent(100, *em.projects_recent_deploy)
        # 点击view打开ops页面
        cp.clicks(10, *em.projects_view_frontend)
        # 获取ops页面的文本信息
        tx1 = cp.find_elements(10, *em.frontend_status_list)[1].text
        tx2 = cp.find_elements(10, *em.frontend_status_list)[2].text
        # 点击链接打开模板展示
        cp.click(10, *em.frontend_ops_domains)
        tx3 = cp.get_frontend_view(case['case_id'])
        print("tx1=" + tx1, "tx2=" + tx2, "tx3=" + tx3)
        new_url = connect_db.current_url
        try:
            assert case['text']['tx3'] in tx1
            assert case['case_data'] in tx2
            assert case['text']['tx5'] in tx3
            assert get_ini_data('url', 'new_url') in new_url
    ...
    @allure.suite('ICP生态删除合约')
    @allure.description("ICP生态删除合约")
